...
- Notes: **rich-text editor** (contentEditable) with a formatting toolbar — bold / italic / underline, three text sizes, six text colours, bullet points, checklists with tickable boxes, and an equation editor. Native Ctrl+B/I/U still work. Working **New note** button and **AI enhance** (real GPT-4o-mini via `/api/enhance`).
...
- **List/checklist structural edits are hand-rolled, not `execCommand`.** `lib/richText.ts` exports `closestOwnBlock`, `isCaretAtBlockStart`, `detachListItem`, `wrapInList`, `placeCaretAtStart`. `execCommand("insertUnorderedList")` toggled off mid-list can leave a bare text node next to the lists instead of a `<p>`, and toggled on inside an existing block can nest a stray `<ul>` *inside* it — both reproduced and fixed this session. Bullets and checklists are mutually exclusive (never both on one block); Backspace at the very start of a bullet or checklist item detaches just that item (splitting the list around it) instead of merging into whatever's above, matching what the toolbar buttons do. Any future block-structure change here should extend these helpers rather than reach for `execCommand` on lists again.
...
- **Sanitiser** (`sanitizeNoteHtml`): everything entering the editor from outside — the enhance response, the explain/refine response, clipboard paste — is stripped to an allowlist of tags/attributes via an inert `DOMParser` document. Scripts, event handlers, inline styles, embeds and bogus `class`/`contenteditable`/`data-done`/`color` values never reach `innerHTML`. The allowlist covers `<sup>`/`<sub>`/`<ul>`/`<ol>`/`<li>` and `<span class="math"|"frac"|"num"|"den"|"sqrt"|"rad"|"sqrt-body" data-tex contenteditable="false">` alongside the original tag set. Keep using it for any future path that writes note HTML.
...
components/workspace/
                ...
                NotesTab, NoteToolbar, EquationEditor, ExplainPanel,
                RecordTab, QuizzesTab, ResourcesTab
lib/            subjects (model + seed), subjectsStore, schedule, subjectColors,
                richText (HTML <-> text + sanitiser), math (equation renderer),
