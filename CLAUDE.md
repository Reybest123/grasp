...
- **Leaving the Record tab mid-recording is confirmed, in-app.** `/home` never changes URL when a workspace tab changes, so there is no navigation for the browser's own unsaved-changes prompt to catch — the guard has to be ours. `RecordTab` reports its phase up through `onPhaseChange`; `SubjectWorkspace` routes every tab button *and* the "All notebooks" back link through `leave()`, which raises the shared `ConfirmDialog` when the phase isn't `idle`. The `naming` phase is guarded as well as `recording`: the lecture is over but the drafted notes aren't in the subject yet, so leaving still destroys them (the copy differs between the two). Saving is the one exit that isn't guarded — `addNote` sets the tab itself and never touches `leave()`. Confirming unmounts `RecordTab`, whose existing cleanup releases the microphone and reports `idle`, which clears the guard. The `beforeunload` handler is unrelated and still covers actually closing/refreshing the page.
...
- A recording still does not *survive* a tab switch — it is now confirmed rather than silently lost (see below). Making it survive means lifting the recorder above the tab router.
