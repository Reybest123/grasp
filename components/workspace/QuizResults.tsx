...
/** A 76px radius behind a 14px stroke, written as the box StatRing takes. */
const SIZE = 170;
const STROKE = 14;

/**
 * What each band is called here, and the tint its mark total sits on. The 0.8 /
 * 0.5 thresholds themselves live in lib/stats.ts alongside the ring colours, so
 * this screen and the dashboard's strength bars cannot come to disagree about
 * what a good score is.
 */
const VERDICT: Record<BandName, { verdict: string; tint: string }> = {
  strong: { verdict: "Strong grasp", tint: "bg-emerald-50 text-emerald-700" },
  fair: { verdict: "Getting there", tint: "bg-amber-50 text-amber-800" },
  weak: { verdict: "Worth another pass", tint: "bg-red-50 text-red-700" },
};
...
export function QuizResults({
  ...
  const { verdict, tint } = VERDICT[band];
  ...
  return (
    <div>
      ...
      <div className="mx-auto max-w-xl animate-[popIn_180ms_ease-out] rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-soft">
        ...
        <h2 className="mt-6 text-2xl font-bold tracking-tight text-ink">{verdict}</h2>
        <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-slate-500">{sub}</p>

        <span
          className={`mt-4 inline-flex items-center rounded-full px-3 py-1 text-sm font-semibold tabular-nums ${tint}`}
        >
